...
export default function HeroSection() {
  ...
  useGSAP(() => {
    ...
    // Subtle background zoom (Cinematic effect)
    gsap.fromTo('.hero-image-zoom', 
      { scale: 1 }, 
      { scale: 1.03, duration: 25, ease: 'sine.inOut', repeat: -1, yoyo: true }
    );
    ...
    for(let i = 0; i < 20; i++) {
      gsap.to(`.particle-${i}`, {
        ...
        repeat: -1,
        yoyo: true,
        delay: Math.random() * -10 // Random negative delay so they start already moving
      });
    }
